// DANIEL NG
//  EFM8LB1 IR Transmitter + LCD Display Controller 
...
#define LCD_D5 P1_4
#define LCD_D6 P1_5
// P1.6 IS SKIPPED IS THE IR TRANSMITTER!
#define LCD_D7 P1_7
...
void main (void)
{
    ...
    // Boot up into State 0 (Main Menu)
    lcd_state = 0; 
    
    while(1)
    {
        // ==========================================
        // STATE 0: MAIN BOOT MENU
        // ==========================================
        if (lcd_state == 0) 
        {
            lcd_command(0x0C); // Ensure cursor is OFF for this menu
            ...
            if (UP == 0) {
                lcd_state = 1; // Switch to AUTO mode
                lcd_command(0x01); // Clear screen for the transition
                lcd_delay_ms(200); // Button debounce delay
            }
            if (DOWN == 0) {
                lcd_state = 2; // Switch to MANUAL mode
        ...
        // ==========================================
        // STATE 1: AUTO MODE (PATH SELECTION)
        // ==========================================
        else if (lcd_state == 1) 
        {
            // 1. Draw the static text
            lcd_set_cursor(0, 0);
            lcd_print("SELECT PATH:    ");
            lcd_set_cursor(1, 0);
            lcd_print("  1    2    3   "); // Numbers are at cols 2, 7, and 12

            // 2. Read the buttons to change the selected path
            if (LEFT == 0) {
                current_path--;
                if (current_path < 1) current_path = 3; // Wrap around to 3
                lcd_delay_ms(250); // Prevent rapid-fire scrolling
            }
            if (RIGHT == 0) {
                current_path++;
                if (current_path > 3) current_path = 1; // Wrap around to 1
                lcd_delay_ms(250); 
            }

            // 3. Turn on the underline cursor and put it under the selected number
            lcd_command(0x0E); // Display ON, Cursor ON (Underline)
            
            if (current_path == 1) {
                lcd_set_cursor(1, 2); // Park cursor under the '1'
            } 
            else if (current_path == 2) {
                lcd_set_cursor(1, 7); // Park cursor under the '2'
            } 
            else if (current_path == 3) {
                lcd_set_cursor(1, 12); // Park cursor under the '3'
            }
            
            // Allow user to press UP to "Confirm" their choice
            // if (UP == 0) { ... start transmitting auto sequence ... }
        }

        // ==========================================
        // STATE 2: MANUAL MODE (DRIVING)
        // ==========================================
        else if (lcd_state == 2) 
        {
            lcd_command(0x0C); // Turn cursor OFF for driving
            ...
            // Here is where you will add your IR send_forward, send_stop, etc. 
            // based on the buttons being pressed!
        }
    }
}
