...
#define GUIDEWIRE_INTERSECTION_CENTER_MIN 1850u
...
#define FORWARD_MS                        8u
#define SOFT_TURN_FORWARD_MS              20u
#define HARD_TURN_PULSE_MS                4u
#define INTERSECTION_REARM_LOOPS          120
#define INTERSECTION_CONFIRM_COUNT        3
#define INTERSECTION_CLEAR_CONFIRM_COUNT  3
#define INTERSECTION_POST_BLOCK_MS        300u
...
static uint32_t intersection_block_until = 0;
static bool forward_release_pending = false;
static int forward_release_confirm = 0;
static bool forward_release_saw_clear = false;
static int forward_track_confirm = 0;
...
static void reset_guidewire_state(void)
{
    ...
    intersection_block_until = 0;
    forward_release_pending = false;
    forward_release_confirm = 0;
    forward_release_saw_clear = false;
    forward_track_confirm = 0;
}
...
static bool handle_intersection_runtime(void)
{
    ...
    intersection_clear_confirm = 0;
    intersection_block_until = millis() + INTERSECTION_POST_BLOCK_MS;

    switch (action)
    {
        case PATH_FORWARD:
            // Tracked forward burst to clear the intersection physically.
            // Uses sensor feedback to stay on the wire (unlike L/R phase 1
            // which can be blind because the turn is intentionally off-wire).
            for (i = 0; i < 25; i++)
            {
            ...
            // Do NOT use the latch for forward.  On a straight wire the
            // sensor readings stay high so center_cleared never fires and
            // the latch stays on forever, suppressing all future detections.
            intersection_latched = false;
            intersection_clear_confirm = 0;
            intersection_confirm = 0;
            intersection_block_until = millis() + 220u;
            recovery_mode = false;
            break;
        ...
        case PATH_LEFT:
            for (i = 0; i < 36; i++)
            {
                ...
                if(guidewire_wait_check_abort(4)) return true;
            }
...
static void run_guidewire_mode(void)
{
    while(current_mode == MODE_GUIDEWIRE)
    {
        ...
        bool center_intersection = (r.center >= GUIDEWIRE_INTERSECTION_CENTER_MIN);
        bool sides_strong = (r.left >= 1430u) && (r.right >= 1430u);
        bool balanced_for_crossing = (abs_diff <= 215);
        bool is_intersection_now = center_intersection && sides_strong && balanced_for_crossing;

        bool center_cleared = (!center_intersection) || (!sides_strong) || (r.center < 900u);
        ...
        if (forward_release_pending)
        {
            bool normal_track_now = (r.center < 900u) && (r.left >= 1700u) && (r.right >= 1700u) && (abs_diff <= 220);

            if ((int32_t)(millis() - intersection_block_until) < 0)
            {
                // Still in the minimum forward suppression window.
            }
            else if (!forward_release_saw_clear)
            {
                if (!is_intersection_now)
                {
                    forward_release_confirm++;
                    if (forward_release_confirm >= 4)
                    {
                        forward_release_saw_clear = true;
                        forward_release_confirm = 0;
                        forward_track_confirm = 0;
                    }
                }
            ...
            else
            {
                if (normal_track_now)
                {
                    forward_track_confirm++;
                    if (forward_track_confirm >= 3)
                    {
                        forward_release_pending = false;
                        forward_release_confirm = 0;
                        forward_release_saw_clear = false;
        ...
        else if (intersection_latched)
        {
            if (center_cleared)
            {
                intersection_clear_confirm++;
                if (intersection_clear_confirm >= INTERSECTION_CLEAR_CONFIRM_COUNT)
                {
                    intersection_latched = false;
                    intersection_clear_confirm = 0;
                    intersection_confirm = 0;
        ...
        else if (diff > 0)
        {
            if (abs_diff > (int32_t)GUIDEWIRE_HARD_TURN_MARGIN)
            {
                ...
                {
                    guidewire_motor_left();
                    if(guidewire_wait_check_abort(8)) break;
                }
            }
            else
            {
